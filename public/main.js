document.addEventListener('DOMContentLoaded', () => {
    console.log("페이지 로딩 완료! main.js 시작! (똑똑한 소리 듣기 버전)");
    ...
    // ⭐⭐⭐ 소리 마법(SpeechSynthesis)을 미리 깨우는 도우미 함수! ⭐⭐⭐
    ...
    function loadVoices() {
        if ('speechSynthesis' in window) {
            ...
            if (voices.length === 0) {
                // 목소리 목록이 바로 안 불러와질 때가 많아요. 이럴 때를 대비해서...
    ...
    // (이하 다른 모든 함수들은 이전 "최종 완성판"과 동일합니다. 맨 아래 이벤트 리스너 부분만 수정됩니다.)
    ...
    if (listenButtonElement) {
        listenButtonElement.addEventListener('click', () => {
            ...
            if ('speechSynthesis' in window) {
                ...
                // 이전에 말하던 게 있으면 일단 멈춰요 (소리가 겹치지 않게)
                ...
                if (koreanVoice) {
                    utterance.voice = koreanVoice; // 찾았다면 그 목소리로 설정!
                    console.log("   ✅ 한국어 목소리를 찾아서 사용해요:", koreanVoice.name);
                } else {
    ...
    initializeGame(); // 페이지 처음 열릴 때 게임 초기화하고 모드 선택 화면 보여주기
});

document.addEventListener('DOMContentLoaded', () => {
    console.log("페이지 로딩 완료! main.js 시작! (타임어택 오류 수정 버전)");
    ...
    function startWordTimer() {
        ...
        wordTimerInterval = setInterval(() => {
            ...
            if (wordTimeLeftInSeconds <= 0) handleWordFailureByTimeOut();
            else if (wordTimeLeftInSeconds <= 5 && wordTimerDisplayElement) wordTimerDisplayElement.style.color = 'orange';
        }, 1000);
    }
    ...
    function startOverallGameTimer() {
        ...
        overallGameTimerInterval = setInterval(() => {
            ...
            if (overallGameTimeLeftInSeconds <= 0) handleOverallTimeUp();
            else if (overallGameTimeLeftInSeconds <= 10 && overallTimerDisplayElement) overallTimerDisplayElement.style.color = '#e74c3c';
        }, 1000);
    }
    ...
    function presentNextWord() {
        ...
        if (currentGameMode === MODE_HOW_MANY) {
            stopWordTimer(); resetWordTimer(); startWordTimer();
        }
    }
    ...
    function handleWordFailureByTimeOut() {
        if (currentGameMode !== MODE_HOW_MANY || !gameIsActive) return;
        stopWordTimer();
        if (isCurrentlyRecording && mediaRecorderTool && mediaRecorderTool.state === 'recording') { isCurrentlyRecording = false; mediaRecorderTool.stop(); } 
    ...
    async function sendVoiceToRobotForGrading(voiceAudioBlob) {
        ...
        // ⭐ "잠시만요..." 동안 "얼마나 많이" 모드의 단어 타이머만 일시정지! ⭐
        if (currentGameMode === MODE_HOW_MANY) stopWordTimer();
        ...
        try {
            ...
            let canRetry = (currentGameMode === MODE_HOW_MANY && wordTimeLeftInSeconds > 0) || (currentGameMode === MODE_TIME_ATTACK && overallGameTimeLeftInSeconds > 0);
            if (canRetry) {
                if(recordButtonElement) { recordButtonElement.textContent = '🔴 녹음 시작'; recordButtonElement.disabled = false; recordButtonElement.classList.remove('recording');}
    ...
    function handleRobotResponse(resultFromServer) {
        ...
        if (!resultFromServer.success) {
            if(feedbackResultArea) feedbackResultArea.innerHTML = `<p style="color: red;">${resultFromServer.errorMessage || '결과 못 받음'}</p>`;
            let canRetry = (currentGameMode === MODE_HOW_MANY && wordTimeLeftInSeconds > 0) || (currentGameMode === MODE_TIME_ATTACK && overallGameTimeLeftInSeconds > 0);
            if (canRetry) {
                if(recordButtonElement) { recordButtonElement.textContent = '🔴 녹음 시작'; recordButtonElement.disabled = false; recordButtonElement.classList.remove('recording');}
                if (currentGameMode === MODE_HOW_MANY) startWordTimer();
        ...
        if (isCorrectAnswer) {
            ...
            if(feedbackResultArea) feedbackResultArea.innerHTML = `<p style="color: orange;">${resultFromServer.feedbackMessage}</p>`;
            let canRetry = (currentGameMode === MODE_HOW_MANY && wordTimeLeftInSeconds > 0) || (currentGameMode === MODE_TIME_ATTACK && overallGameTimeLeftInSeconds > 0);
            if (canRetry) {
                if(recordButtonElement) { recordButtonElement.textContent = '🔴 녹음 시작'; recordButtonElement.disabled = false; recordButtonElement.classList.remove('recording');}
                if (currentGameMode === MODE_HOW_MANY) startWordTimer();
    ...
    if (listenButtonElement) {
        listenButtonElement.addEventListener('click', () => {
            ...
            if ('speechSynthesis' in window) {
                ...
                utterance.lang = 'ko-KR'; utterance.rate = 0.85; utterance.pitch = 1; 
                let koreanVoice = voices.find(voice => voice.lang === 'ko-KR');
                if (koreanVoice) utterance.voice = koreanVoice;
                utterance.onerror = (event) => { console.error("SpeechSynthesis Error:", event.error); alert(`소리 내기 오류: ${event.error}`);};
                window.speechSynthesis.speak(utterance);
            } else { alert("이 브라우저에서는 음성 합성을 지원하지 않습니다."); }
        });
    }
    ...
    if (recordButtonElement) {
        recordButtonElement.addEventListener('click', async () => {
            ...
            if (currentGameMode === MODE_TIME_ATTACK && overallGameTimeLeftInSeconds <= 0) return;
            if (currentGameMode === MODE_HOW_MANY && wordTimeLeftInSeconds <= 0) return;
            if (!isCurrentlyRecording) {
                try {
                    ...
                    mediaRecorderTool.addEventListener('stop', () => {
                        ...
                        let canSend = (currentGameMode === MODE_HOW_MANY && wordTimeLeftInSeconds > 0) || (currentGameMode === MODE_TIME_ATTACK && overallGameTimeLeftInSeconds > 0);
                        if (gameIsActive && canSend && completeAudioBlob.size > 0) {
                            ...
                            if(feedbackResultArea) feedbackResultArea.innerHTML = "<p>앗! 녹음된 목소리가 없어요.</p>";
                            if (canSend) { recordButtonElement.textContent = '🔴 녹음 시작'; recordButtonElement.disabled = false; recordButtonElement.classList.remove('recording'); isCurrentlyRecording = false; if (currentGameMode === MODE_HOW_MANY) startWordTimer(); }
                        }
                    });
                    mediaRecorderTool.start();
                    recordButtonElement.textContent = '⏹️ 녹음 중지'; recordButtonElement.classList.add('recording');
                    if(feedbackResultArea) feedbackResultArea.innerHTML = "<p>지금 말해보세요...🎙️</p>";
                    isCurrentlyRecording = true;
                } catch (error) { console.error("마이크 오류:", error); alert("마이크 사용 불가!"); if(feedbackResultArea) feedbackResultArea.innerHTML = "<p>마이크 사용 불가 😭</p>"; recordButtonElement.textContent = '🔴 녹음 시작'; recordButtonElement.classList.remove('recording'); isCurrentlyRecording = false;}
            } else { 
                if (mediaRecorderTool && mediaRecorderTool.state === 'recording') {
                    // "잠시만요..." 동안 타이머 일시정지 (타임어택 제외)
                    if (currentGameMode === MODE_HOW_MANY) stopWordTimer();
                    isCurrentlyRecording = false; 
    ...
    function shareGameResult() {
        if (!finalMessageElement || !finalScoreDisplayElement) return;
        let gameModeText = "";
        if (currentGameMode === MODE_HOW_MANY) gameModeText = "🚀 얼마나 많이 통과?";
        else if (currentGameMode === MODE_TIME_ATTACK) gameModeText = "⏱️ 60초 타임어택!";
        const titleToShare = "✨ 한국어 발음왕 도전! 내 결과 좀 봐! ✨";
        const textToShare = `모드: ${gameModeText}\n결과: ${finalMessageElement.textContent}\n${finalScoreDisplayElement.textContent}\n\n같이 도전해봐! 👇\n#한국어발음왕 #발음챌린지`;
        const urlToShare = window.location.href;
        const shareData = { title: titleToShare, text: textToShare, url: urlToShare };
        if (navigator.share) {
            try { navigator.share(shareData); }
            catch (err) { console.error('공유 실패:', err); copyToClipboardFallback(titleToShare + "\n" + textToShare + "\n" + urlToShare); }
        } else {
            console.log('Web Share API 지원 안됨 - 클립보드로 복사합니다.');
            copyToClipboardFallback(titleToShare + "\n" + textToShare + "\n" + urlToShare);
        }
    }

    function copyToClipboardFallback(textToCopy) {
        navigator.clipboard.writeText(textToCopy)
            .then(() => alert("게임 결과가 복사되었어요! SNS에 붙여넣고 자랑해보세요! 📋🎉"))
            .catch(err => { console.error('클립보드 복사 실패:', err); alert("앗! 결과 복사도 실패했어요. 😥"); });
    }

    // --- 페이지 로드 시 버튼 이벤트 리스너 연결 및 게임 초기화 ---
    if (startHowManyButtonElement) startHowManyButtonElement.addEventListener('click', () => {
        currentGameMode = MODE_HOW_MANY;
        showScreen('levelSelection');
    });
    if (startTimeAttackButtonElement) startTimeAttackButtonElement.addEventListener('click', () => {
    ...
    if (restartGameButtonElement) restartGameButtonElement.addEventListener('click', () => { if(currentGameMode) startGame(currentGameMode); else initializeGame(); });

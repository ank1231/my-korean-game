document.addEventListener('DOMContentLoaded', () => {
    ...
    function presentNextWord() { if (!gameIsActive) return; if (currentWordIndex >= currentWordList.length) { if (currentGameMode === MODE_HOW_MANY) { handleGameClear(); return; } if (currentGameMode === MODE_TIME_ATTACK) { currentWordIndex = 0; } } currentWordToPractice = currentWordList[currentWordIndex]; if (wordDisplayElement) wordDisplayElement.textContent = currentWordToPractice; if (feedbackResultArea) feedbackResultArea.innerHTML = "<p>발음해보세요!</p>"; if (recordButtonElement) { recordButtonElement.textContent = '🔴 녹음 시작'; recordButtonElement.disabled = false; recordButtonElement.classList.remove('recording');} isCurrentlyRecording = false; if (currentGameMode === MODE_HOW_MANY) { stopWordTimer(); resetWordTimer(); startWordTimer(); } }
